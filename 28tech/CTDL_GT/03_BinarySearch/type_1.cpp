...
// Type 1: Các phần tử trong mảng là khác nhau, và sắp xếp tăng dần
...
// Type 1.3: Tìm index phần tử nhỏ nhất thỏa mãn điều kiện biểu thức EX
int binarySearch3(int arr[], int size, int x) {
    int l = 0, r = size - 1;
    while (l <= r)
    {
        int m = l + (r-l)/2;
        if (arr[m] < x) {      // giả sử điều kiện biểu thức EX: thỏa mãn >= giá trị x     ==> upper_bound
            l = m + 1;
        } else {
            r = m - 1;
        }
